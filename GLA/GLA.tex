...
\begin{figure}[!ht]
	\centering
	\scalebox{1.3}{\input{GLA/HVSPS.tex}}
...
\begin{figure}[!ht]
	\centering
	\scalebox{1.3}{\input{GLA/RateSPS.tex}}
...
\begin{figure}[!ht]
	\centering
	\scalebox{1.315}{\input{GLA/MultiplicitySPS.tex}}
...
\marginpar
{
	...
	\captionof{figure}{Le passage dans les chambres d'une muon et de photons simulé par GEANT4.}
	\label{GEANT4}
}
...
Au vu de l'état des chambres, il a été décidé de construire de nouvelles chambres de basse résistivité de taille \SI{32}{\centi\meter}$\times$\SI{30}{\centi\meter}. Malheureusement seul \num{4} verres nous sont arrivé en bonne états, ce qui permit de ne créer que deux nouvelles chambres. Ce nombres étant insuffisants pour une étude du vieillissement, nous avons décider de les utiliser afin d'instrumenter une chambre double gap avec l'électronique présentée figures \ref{DoubleGap} et \ref{SchemePS}. Un même type de PCB à été utilisé afin de créer une chambre avec des électrodes en Bakélite de \SI{2}{\milli\meter} d'épaisseur et un gap de \SI{2}{\milli\meter}. Les résultats de ces tests sont présentés section \ref{smallchamber}.
...
\begin{figure}[!ht]
	\centering
	\scalebox{1.3}{\input{GLA/BigChambersComparaison.tex}}
...
\begin{figure}[ht!]
	\centering
	\subfloat[Timestamps des strips non-alignés.]{\scalebox{0.75}{\input{GLA/fitting_unaligned.tex}}\label{nonalign}}
	\\
	\subfloat[Timestamps des strips alignés.]{\scalebox{0.75}{\input{GLA/fitting_aligned.tex}}\label{align}}
...
\begin{figure}
\centering
\subfloat[$\epsilon_{vrai} -\epsilon_{1}$.]{\scalebox{0.75}{\input{GLA/biais1.tex}}\label{biaismethod1}}
\\
\subfloat[$\epsilon_{vrai} -\epsilon_{2}$.]{\scalebox{0.75}{\input{GLA/biais2.tex}}}
...
\begin{figure}
	\centering
	\scalebox{0.75}{\input{GLA/std.tex}}
...
La chambre à donc subit quelques modification notamment afin d'améliorer la compatibilité électromagnétique (CEM) \cite{charoy2012comp}:
...
\begin{figure}
	\centering
	\scalebox{1.3}{\input{GLA/TimeDistryBC.tex}}
...
\end{figure}
 ...
 \begin{figure}
 	\centering
 	\scalebox{1.3}{\input{GLA/SigmoidBC.tex}}
...
\begin{figure}
	\centering
	\scalebox{1.3}{\input{GLA/ClusterSizeBC.tex}}
...
\begin{figure}
	\centering
	\scalebox{1.3}{\input{GLA/ProbaStreamerBC.tex}}
...
\begin{minipage}[th!]{1\textwidth}
	...
	\begin{minipage}[th!]{0.48\textwidth}
		...
		\begin{minipage}[th!]{1\textwidth}
			...
			\captionof{subfigure}{Polymèrisation des produits de réaction du gaz.}
			\label{depot2}
		\end{minipage}%
	\end{minipage}%
	\captionof{figure}{Différents problèmes rencontrès à l'ouverture de la chambre.}
	\label{probs}
\end{minipage}%

 À certains endroit, le verre à même était attaqué. Ceci pourrait expliquer le bruit important (\SI{400}{\hertz\per\square\centi\meter}) et semble compatible avec les observations faites avec les chambres de petites dimension. Cependant, la circulation du gaz était régulière et les tuyaux était en cuivre afin d'éviter la contamination du gaz par l'humidité. Un test de l'humidité sur la ligne de gaz n'a pas révélé de contamination (\num{750}ppm pour notre ligne sèche contre \num{12000}ppm pour la ligne humide). Ceci nous à amené à penser que le gaz de CMS n'était peut être pas adapté à ces chambres. De plus nos collègues chinois nous ont informé que le taux de \chemform{SF_6} était peut-être trop faible pour ces verres. L'attaque du verre par le gaz a dû être accéléré par les valeurs du courant lorsque nous avont poussé les valeurs de la haute tension ($>$\SI{10}{\kilo\volt}) à comparer à la valeur de fonctionnement typique de \SI{7}{\kilo\volt}.
 
 \section{Étude du bruit et du courant en fonction du pourcentage de \texorpdfstring{\chemform{SF_6}}{SF6}}
 
 Afin de faire les tests de bruit en fonction du pourcentage de \chemform{SF_6} dans le mélange gazeux, la chambre a été démonté et les verres nettoyé de leur peinture puis retournés et repeint de l'autre côté afin que le côté abimé des verres soit en dehors de gap. Le PCB a été changé et une couche de mylar a été inséré entre les verres et celui-ci. De plus, lors des test après montage, nous avons constater des création de plasma entre les couches des verres sur les bords de ceux-ci. Un film de kapton a donc était ajouté le long des bords des verres. Le gaz utilisé est celui de ILC (\chemform{TFE}, \chemform{CO_2}, \chemform{SF_6}). En parallèle, un test avec une nouvelle chambre $\num{30}\times\num{30}$ utilisé au GIF++ a été effectué au CERN avec le gaz de CMS, l'électronique basé sur les HARDROC et le PCB à strips.
 
 La chambre de type RE1/1 a été amené au banc de test cosmique précédement décrit section \ref{banc}. La carte de test est la même que précédement. Cependant afin de mesurer le taux de bruit de la chambre, la coincidence des scintillateur à été remplacé par un générateur de fonctions mimant le signal de la coîncidence des scintillateurs à haute fréquence afin que celle-ci soit largement supérieur au taux de muons traverssant la chambre. Les hits ainsi enregistrer sont donc majoritairement des hits de bruit. Le taux de bruit estimé pour un strip "n" est donné par la formule :
 \begin{equation}
 N_n^{bruit}=\frac{N_n^{hits}}{N_{trigger}T}
 \end{equation}
 où $N_n^{hits}$ est le nombre de hits enregistrer pendant la prise de mesure, $N_{trigger}$ le nombre de "false trigger" produit par le générateur de fonction et $T$ la durée d'une fenêtre d'acquisition.
 
 La figure \ref{CurrentDOME} montre le courant dans la chambre de type RE1/1 en fonction du pourcentage de \chemform{SF_6} dans le mélange de gas. Pour les mesures prise sur le banc cosmique un ajustement des tensions en tenant compte de la variation de la température et de la pression est effectué. Cette ajustement applique la même formule que celle de CMS. Les valeurs des courant sont les moyennes de 30 mesures. A titre de comparaison, le courant de la chambre quand celle-ci était au GIF++ avec la source OFF est également présent. Cependant, ces courant ne sont pas moyenné et aucun ajustement de la tension n'était effectué.
 
\begin{figure}[ht!]
	\centering
	\scalebox{1.2}{\input{GLA/CurrentRE11.tex}}
...
Comme prévu, le \chemform{SF_6} étant un gaz très électronégatif est servant à controller l'excés d'électrons lors d'une avalanches, moins celui-ci est abondant dans le mélange de gaz est plus le courant dans la chambre augment. Cela est dû au fait que les avalanches contiennent plus d'électrons car ceci n'est sont pas réabsorbé. La probabilité de streamer augmente également, accroissant le courant dans les chambres. Un facteur de $\sim 3-4$ est constaté entre le courant de la chambre avec le mélange à 5\% de \chemform{SF_6} et celui pour le mélange à \num{0.3}\% de \chemform{SF_6}.

Le courant mesuré pour la chambre de basse résistivité de taille $\num{30}\times\num{30}$ donne des résultats similaire lorsque l'on tiens compte de sa surface plus petit que celle de la chambre RE1/1.
...
Le taux de bruit moyen pour la Partition B de la chambre RE1/1 en fonction de la tension pour différentes proportions de \chemform{SF_6} est donnée figure \ref{bruitB}.
 ...
 Le bruit a été réduit par rapport à la période de tests en faisceaux. Ceci peut être dû à une meilleur compatibilité électromagnétique ainsi que de la pose du kapton le long des verres. Comme attendu le taux de bruit augment à mesure que baisse la proportion de \chemform{SF_6}. Pour le gaz avec 5\% de \chemform{SF_6}, le taux de hits de bruit est compris entre \num{10} et \num{20} \si{\hertz\per\square\centi\meter}.
...
\begin{figure}[ht!]
	\centering
	\subfloat[]{\includegraphics[width=0.75\textwidth]{GLA/Noise5p100.png}} 
	\hfill
	\subfloat[]{\includegraphics[width=0.75\textwidth]{GLA/Noise3p100.png}}
	\caption{Taux de bruit dans la partition B de la chambre RE1/1 en fonction de la tension appliquée pour différentes seuils. La proportion de \chemform{SF_6} est de 5\% (haut) et 3\% (bas).}
    \label{bruitB2}
\end{figure}  
  
Pour les chambres placés dans le banc de test au CERN, un scan en tension a été effectué afin de mesurer le taux de bruit est l'efficacité des chambres.

La figure \ref{effi904Chi} (\ref{effi904Bak}) montre l'efficacité et le taux de bruit pour la chambre en verre de basse résistivité (en bakélite) en fonction de la tension appliqué pour le mélange de gas CMS. Le seuil appliqué est de \SI{72}{\femto\coulomb}. Dans le cas de la chambre en verre une étude de l'éfficacité en fonction de la haute tension appliqué pour un proportion de 5\% de \chemform{SF_6} a également été réalisé (courbe rouge sur la figure \ref{effi904Chi}).
 
\begin{figure}[ht!]
	\centering
	\subfloat[]{\scalebox{1}{\input{GLA/EffiNoiseChi.tex}}\label{effi904Chi}} 
	\hfill
	\subfloat[]{\scalebox{1}{\input{GLA/EffiNoiseBak.tex}}\label{effi904Bak}}
	\caption{Efficacité et taux de bruit pour la chambre en verre de basse résistivité (haut) et en Bakélite (bas) pour le mélange de gas CMS (noir) et le mélange à 5\% de \chemform{SF_6}. Le seuil appliqué est de \SI{72}{\femto\coulomb}.}
\end{figure}  

Les deux chambres présentent une bonne efficacité de détection des muons. La chambre bakélite fonctionne parfaitement avec l'électronique à base de HARDROC et a très bas seuils. Cependant la chambre en verre de basse résistivité a un taux de bruit nettement supérieur à la chambre en Bakélite ($\sim$\num{7}). Les résultats du taux de bruit de la chambre de basse résistivité semblent confirmer ceux effectuer en parallèle à Lyon. Le taux de \chemform{SF_6} est un élément très importants pour les électrodes en verre de basse résistivité, vers le point de fonctionnement (\SI{7000}{\volt}) le taux de bruit pour un mélange composé de \num{0.3}\% de \chemform{SF_6} est multiplié par \num{5} par rapport au mélange avec \num{5}\% de \chemform{SF_6}. De plus, il semble rester des problèmes de CEM pour la chambre RE1/1.


\section{Tests en Faisceaux au GIF++ des chambres \texorpdfstring{\num{32}$\times$\num{30}\si{\square\centi\meter}}{30cm x 30cm} à strips}
\label{smallchamber}

Les deux chambres de \SI{32}{\centi\meter}$\times$\SI{30}{\centi\meter} double gap à strips (cf. figs \ref{DoubleGap}, \ref{SchemePS} et \ref{nouvellestrip}) en verre de basse résistivité et en Bakélite ont été inséré dans le bâti des anciennes chambres de basse résistivité \SI{32}{\centi\meter}$\times$\SI{30}{\centi\meter} (cf.fig~\ref{bati}) en octobre 2017. Les nouvelles chambres utilisent l'éléctronique HARDROC, tout comme les anciennes encore présentes sur le bâti. Cependant, il a été décidé de ne pas utiliser le programme développé section \ref{prog}. En effet, comme celui-ci repose sur la création de traces, afin de calculer l'efficacité des chambres, les anciennes chambres sont nécessaire à leurs création. Cependant, au vu des vieillissements des chambres constatés, nous avons décidé de ne pas utilisé les données récoltés par celles-ci.
...
Afin de calculer l'efficacité des nouvelles chambres, le programme développer pour la chambre RE1/1, basé sur le comptage sera utilisé. Les scintillateurs nécessaire à la détection du passage des muons sont ceux fournis par nos collègues du GIF++. Les coïncidences des scintillateurs sont incluses dans les données grâce à une DIF configurée de telles manière à enregistrer ces coïncidences comme le passage d'un seuils d'un pads (cf.tab~\ref{encodage}) (seuil0 pour le déclenchement d'un scintillateur, seuil1 pour le déclenchement de l'autre, et seuil2 pour la coïncidences des deux scintillateurs). Il est ainsi possible, de sélectionner une fenêtre temporelle autour de ces coïncidence, en tenant compte dû décalage temporelle dû à la longueur des câbles portant le signal des scintillateurs (cf.fig~\ref{decalage}). 
...
\begin{figure}[ht!]
	...
	\caption{Distribution du nombres de hits en fonction de numéro du strip pour divers temps avant le signal de la coïncidence des scintillateurs reçue dans les données LCIO.}
	\label{decalage}
\end{figure}  

Les hits correspondant au passage du faisceau arrive donc entre  \SI{1.2}{\micro\second} et \SI{1.6}{\micro\second} avant les données de coïncidence des scintillateurs. Une fenêtre de temps centré sur le temps $t_{scinti}-t_{retard}$ avec $t_{retard}=\SI{1.4}{\micro\second}$ est ensuite extrait des données LCIO pour chaque trigger. Un convertisseur permet ensuite de passer les données du format LCIO au format ROOT. Il est ainsi possible de tourner sur les données des HARDROC comme s'il s'agissait de données venant de l'électronique de CMS.

Ce test en faisceau a démontré l'utilité du elog. En effet, un programme en PYROOT a été réalisé afin de prendre en entrée les données du elog sous forme CSV est de généré des graphiques permettant de suivre les runs pris et leur paramètre et d'améliorer la décision des type de runs qu'il restait à prendre (cf.fig~\ref{elogutil}). Le programme a permis également de créer automatiquement les listes de runs à utiliser selon le type de graphique à produire. 

\begin{figure}[ht!]
	\centering
	\subfloat[Distribution du nombres de runs en fonction du nom de la configuration dans la database. \num{30} runs ont été écrit dans le mauvais elog (M3) suite à une erreur de configuration.]{\includegraphics[width=0.48\textwidth]{GLA/DistrDatabase.png}} 
	\hfill
	\subfloat[Corrélation entre le status du faisceau et le status de la source pour les \num{126} runs ayant été inscrit dans le bon elog.]{\includegraphics[width=0.48\textwidth]{GLA/hStatusBeamVsSource.png}} 
	\\
	\subfloat[Nombre de runs en fonction du seuil, l'axe $z$ est le nombre d'atténuateur de la source pour les threshold correspondant.]{\includegraphics[width=0.48\textwidth]{GLA/hAttScanThresholdVsNrun.png}} 
	\hfill
	\subfloat[Valeur de l'atténuateur de la source en fonction du seuil, la couleur (le numèro) de l'axe $z$ reprèsente la valeur de l'atténuateur (le nombre de runs pris avec cette valeur d'atténuateur) pour le seuil correspondant.]{\includegraphics[width=0.48\textwidth]{GLA/hAttScanThresholdVsAtt.png}}
	\caption{Différents graphiques produit par le programme écrit en PYROOT à partir des données fournit par le ELOG dans le format CSV. Sur les axes PAD1(3,250L) par exemple représente le nom de la chambre ici PAD1, le numéro du seuil ($i'=$1,2 ou 3) et la valeur $\mbox{DAQ}_{i=i'-1}$ appliquée (250) (cf.fig~\ref{equationn}).}
	\label{elogutil}	
\end{figure}

\subsection{Efficacité en fonction du seuil}
Le résultat principal de ce test en faisceaux est l'efficacité en fonction du seuil appliqué à la chambre de verre standard (en Bakélite) montré figure \ref{effithrchi} (\ref{effithrbak}). Ceux-ci peuvent être réalisé grâce à la grande plaque de seuils applicable au HARDROC.
...
Les deux chambres ont un plateau proche de \num{95}\%. Ce test à permis de démontrer que l'électronique de CMS fonctionne correctement avec les chambres composées d'électrodes en Bakélite.

L'intégration de ces courbes permet d'en déduire la charge moyenne récoltée par l'électronique lors d'un trigger. On remarque que la charge récoltée est environ \num{7.5} fois plus importante pour la chambre en verre (\SI{4.2}{\pico\coulomb}) que celle en Bakélite (\SI{0.56}{\pico\coulomb}).

\section{Conclusion}

Les chambres constituées d'électrodes de verre de basse résistivité ont été testées lors de nombreux tests en faisceaux (DESY, PS, SPS, GIF++). Tous ces tests ont montrés que ces chambres sont capables de supporter des flux de particules importants tout en ayant une très bonne efficacité. Deux types de mélanges gazeux ont été testé (ILC et CMS) ainsi que plusieurs mélange comportant diverses proportion de \chemform{SF_6}. La proportion de \chemform{SF_6} s'avère être un facteur important du bruit des chambres construit avec ces verres. Dans le cas du mélange de CMS (\num{0.3}\% de \chemform{SF_6}) le taux de bruit est de l'ordre de \SI{50}{\hertz\per\square\centi\meter}. Le vieillissement au GIF++ a montré les limites de notre installation de gas. Une meilleur installation a été réalisé, mais les chambres avait déjà souffert de notre mauvaise installation. Deux solutions ont été apporter afin de construire des chambres de grandes dimensions (RE1/1) avec ces verres dont la dimension ne peut excéder \num{32}$\times$\num{30}\si{\square\centi\meter}. La méthode par fixation mécanique a été retenue car elle n'utilise pas de colle qui pourrait se dégrader au fil du temps et des radiations. L'étanchéité de cette chambre est également assuré par la cassette elle-même permettant ainsi le gonflement de la couche de gaz entre les électrodes. Cependant, un étude plus approfondie de la CEM de cette chambres est nécessaire afin de réduire le bruit. Cette chambre à cependant montré une très bonne efficacité ($\sim$\num{90}\%) même à un flux de $gamma$ de \SI{3}{\kilo\hertz\per\square\centi\meter}, soit \num{3} fois le flux demandé dans le TDR. Finalement, des tests ont montré que l'électronique à base de HARDROC fonctionnée parfaitement avec des chambres en Bakélite.