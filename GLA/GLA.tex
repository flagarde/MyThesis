...
Des mesures du courant des chambres en fonction du facteur d'atténuation ont été effectué afin d'avoir une idée plus précise du comportements des chambres en focntion du flux de particules (cf.fig\ref{courant})
\begin{figure}[ht!]
	\centering
	\subfloat[Chambre "float glass".]{\includegraphics[width=.45\linewidth]{GLA/current_cha4.png}}
	\hfill
	\subfloat[Chambre de basse resistivité (la plus proche de la source).]{\includegraphics[width=.45\linewidth]{GLA/current_cha5.png}}
	\caption{Courant (\si{\micro\ampere}) parcourant les chambres en fonction de la haute tension appliqué pour différents facteur d'atténuation.}
	\label{courant}
\end{figure}
...
Le GIF++ permet également de faire circuler dans les chambres de l'argon afin de les purger et de les nettoyer. L'argon permet également de mesurer la résistivité des électrodes des chambres. Grâce à se gas, les chambres fonctionnent rapidement en mode streamer, le a résistance de la couche de gas devient négligeable  par rapport à la résistance des électrodes due à la création de plasma. La figure \ref{ScanArgon} montre la variation du courant en fonction de la tension appliquée sur les chambres sous Argon. Un ajustement linéaire est effectué afin d'en déduire la conductance des électrodes.
...
La résistivité des chambre de basse resistivité sont beaucoup plus élevé qu'attendu ($\sim$\SI{e10}{\ohm.\centi\meter}). Toutes ces constatations (fig.\ref{struc}, fig.\ref{resistivite}) nous ont amené à ouvrir une chambre afin d'essayer de comprendre ces résultats.
