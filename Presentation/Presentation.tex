   ...
   \visible<3>
	...
	\begin{columns}
		...
		\begin{block}{Caractéristiques}
			\begin{itemize}
				\item {\only<2>{\color{red}}Dimension maximale} : {\only<2>{\color{red}}\SI{32}{\centi\meter}$\times$\SI{30}{\centi\meter}}
				\item {\only<2>{\color{green}}Résistivité volumique} : {\only<2>{\color{green}}\SI{e10}{\ohm.\centi\meter}}
				\item {\only<2>{\color{green}}Épaisseur} : {\only<2>{\color{green}}\SIrange{0.5}{2}{\milli\meter}}
				\item {\only<2>{\color{green}}Uniformité de l'épaisseur} : {\only<2>{\color{green}}\SI{0.02}{\milli\meter}} 
				\item Constante diélectrique : \SIrange{7.5}{9.5}{}
				\item {\only<2>{\color{green}}Rugosité} : {\only<2>{\color{green}}$<$\SI{10}{\nano\meter}}
				\item {\only<2>{\color{green}}Comportement ohmique} : {\only<2>{\color{green}} stable (\SI{1}{\coulomb})}
			\end{itemize}
		\end{block}
		\column{.3\textwidth}
		\begin{center}
...
\end{frame}
    ...
    \begin{block}{\centering Éléments importants pour la caractérisation d'une chambre}
    	\begin{itemize}
    		...
    		\item Sensibilité au bruit de fond (photons, neutrons)
	...
	\begin{frame}
		\begin{columns}
			\column{.6\textwidth}
			\begin{block}{Test du programme d'analyse}
	...
	\subsection{Premiers résultats à DESY (janvier 2012)}
		\begin{frame}
			...
			\begin{block}{\centering Chambres}
				\begin{itemize}
						\item 5 chambres (1 "float glass", 4 "Basse Résistivité")
	...
	\begin{block}{\centering Efficacité en fonction du flux de muons}
		...
		L'Éficacité du verre "standard" chute brutalement $\sim$40\% à \SI{0.1}{\kilo\hertz\per\square\centi\meter}.
	\end{block}}
	\only<2>{
		...
		\begin{columns}
			...
			\begin{block}{\centering Le GIF++}
				\begin{itemize}
					...
					\item Faisceau de muons \SI{100}{\giga\eV}
	...
	%\begin{block}{\centering Valeur de l'atténuateur}
	%	\begin{center}
	%	\includegraphics[width=0.6\textwidth]{DIF32_HR_4.png}
	%	\end{center}
	%\end{block}
	...
	\begin{frame}
		\begin{block}{Courant parcourant les chambres en l'inverse de la valeur d'atténuation}
			\includegraphics[width=0.8\textwidth]{current_same_HV.png}
		\end{block}
		La saturation arrive autour d'un facteur d'atténuation 68.1 pour la float glass alors que la chambre de verre de basse résistivité ne semble pas saturer. 
	\end{frame}
	\begin{frame}
	...
	\begin{columns}
		...
		$e$ : épaisseur totale des électrodes.
		\column{.45\textwidth}
		\begin{tabular}{|c|c|}
			\hline 
		...
		\end{tabular} 
	\end{columns}
{\color{red} La résistivité des verres de basses résistivité est beaucoup plus élevé qu'attendue $sim$\SI{1e10}{\omega\centi\meter}}	
\end{block}	
	...
	\begin{columns}
	\column{.40\textwidth}
	\begin{block}{Ouverture d'une chambre}
		\begin{center}
	...
	Explique :
	\begin{itemize}
	\item La résistivité élevé
	\item Les distribution de hits dans l'analyse
	\\
	...
	\end{center}
		...
		\begin{block}{\centering Méthode par fixation mécanique}
			\begin{columns}
			\column{.45\textwidth}
			\includegraphics[width=0.5\textwidth]{meca1.jpg}
			\column{.45\textwidth}
...
\begin{columns}
	...
	\begin{block}{\centering Setup}
		\begin{itemize}
			...
			\includegraphics[width=0.5\textwidth]{ChamberFloat.png}
...
\begin{block}{\centering Méthode de suppression du bruit}
	...
	\end{center}
		...
		\begin{itemize}
			...
			\visible<2->{\item Ajustement de la distribution des hits en fonction du timestamp par une Gaussienne+constante}
			\only<2-3>{\begin{center}\scalebox{0.3}{\includetex{fitting_aligned}}\end{center}}
			\visible<3->{\item Sélection de la fenêtre [$\mu$-3$\sigma$,$\mu$-3$\sigma$]}
		\end{itemize}
	...
	\vspace*{-0.3cm}
	\begin{itemize}
	\visible<5->{\item Calcule de l'efficacité dans la zone du signal $\epsilon^{signal}$}
	\visible<6->{\item Découpe de la partie temporelle avant le signal en intervalles $I_{n}=\left[t_{n}-t_{win},t_{n}+t_{win}\right]$}
	\visible<7->{\item Calcul de l'efficacité $\epsilon^{bruit}$ et du taux moyens de bruit $\overline{N^{hits}_n}$ dans ces intervalles.}
	\visible<8->{\item Sélection de $I_{n}$ pour lequel $\overline{N^{hits}_n}$ est maximale.}
	\visible<9->{\item Calcul de l'estimateur de l'efficacité :$\epsilon=\frac{\epsilon^{signal}-k\epsilon^{bruit}}{1-k\epsilon^{bruit}}$\\
	\begin{equation}
...
\begin{overlayarea}{0.99\textwidth}{8cm}
	...
	\begin{block}{\centering Biais de l'estimateur $\epsilon$}
		...
		\scalebox{0.4}{\includetex{biais2}}
	\end{block}}
...
\begin{frame}
\begin{block}{Efficacité en fonction de la HV}
	\begin{center}
	\scalebox{0.8}{\includetex{SigmoidBC}}
	\end{center}
\end{block}
\end{frame}
\begin{frame}
\begin{block}{Cluster Size en fonction de la tension}
	\begin{center}
	\scalebox{0.8}{\includetex{ClusterSizeBC}}
	\end{center}
\end{block}
\end{frame}
\begin{frame}
\begin{block}{Probabilité de streamer}
	\begin{center}
		Probabilité définit comme la probabilité d'avoir un cluster de taille supérieur à 7.
	\end{center}
\begin{center}
	\scalebox{0.8}{\includetex{ProbaStreamerBC}}
	\end{center}
\end{block}
\end{frame}
...
\begin{columns}
	\column{0.5\textwidth}
	\begin{block}{Taux de bruit dans la chambre RE1/1 vs \% SF6}
		\scalebox{0.4}{\includetex{CurrentRE11}}
	\end{block}
	\column{0.5\textwidth}
	\begin{block}{Taux de bruit dans la double gap 32$\times$30\si{\square\centi\meter} vs \% SF6}
...
\end{frame}
	...
	\begin{frame}
	\begin{block}{Principe de la nouvelle Électronique}
	...
	\end{itemize}
	\includegraphics[width=0.4\textwidth]{PCB1.png}
	\end{block}
	\end{frame}
	...
	\begin{frame}
		...
		\begin{columns}
			\column{0.45\textwidth}
				...
				\includegraphics[width=0.4\textwidth]{PCB2.png}
				\begin{itemize}
					\item \SI{50}{\centi\meter} de long
					\item 32 strips espacés de \SI{4}{\milli\meter}
					\item 2 PETIROC qui lisent les strips de chaque côté. (32 vois chacun)
					\item 2 TDC (24 voies )
				\end{itemize}
			\end{block}
			\column{0.45\textwidth}
				\begin{block}{Électronique}
					\begin{itemize}
						\item 2 PETIROC qui lisent les strips de chaque côté. (32 vois chacun)
						\includegraphics[width=0.4\textwidth]{PETIROC.png}
						\item 2 TDC (24 voies chacun)
						\includegraphics[width=0.4\textwidth]{TDC.png}
					\end{itemize}
				\end{block}
		\end{columns}
	...
	\begin{frame}
	\begin{block}{Résolution temporelle du PCB}
		\begin{itemize}
			...
			\scalebox{0.6}{\includetex{TDCTimeResolution}}
		\end{itemize}
	\end{block}
\end{frame}
...
\begin{columns}
	...
	\begin{block}{Setup }
		\begin{itemize}
			\item Chambre Bakélite gap:\SI{1.4}{\milli\meter} électrode:\SI{1.4}{\milli\meter}
			...
			\includegraphics[width=0.4\textwidth]{setup2017.jpg}
		\end{itemize}
	\end{block}
	\column{0.5\textwidth}
	\begin{block}{Résolution temporelle T2-T1}
		\includegraphics[width=0.6\textwidth]{25p5cm.jpg}
	\end{block}
\end{columns}
\end{frame}
\subsection{Résolution Spatiale}
\begin{frame}
\begin{block}{Résolution Spatiale}
	\includegraphics[width=0.8\textwidth]{MeanT_Pos.jpg}
...
\end{frame}
	\section{Conclusion}
	\begin{frame}
	\begin{block}{Conclusion et perspective}
		Blabla
	\end{block}
\end{frame}
\end{document}
