...
\begin{document}
	...
	\begin{columns}
		...
		\end{block}
	\end{columns}
	\begin{block}{Découverte du boson de Higgs en 2012 au LHC}
		\begin{itemize}
			\item $m_{h}=\SI{125.2}{\giga\eV}$
			\item Spin 0
		\end{itemize}
	\end{block}
\end{frame}
	...
	\begin{columns}
		...
		\begin{block}{\centering Le LHC}
			\begin{itemize}
				...
				\item 1 croisement de faisceaux toutes les \SI{25}{\nano\second}
    ...
    \begin{columns}
    	\column{.6\textwidth}
   	\begin{block}{\centering Le High Luminosity LHC  (2023-2035)}
   	Augmenter la statistique d'un facteur $\sim$10 dans un temps raisonnable $\sim$10 ans.\\
   	...
   	\begin{itemize}
   		...
   		\item $\mathcal{L}$ la luminosité instantanée.
   	\end{itemize}
     	\end{block}
   \column{.5\textwidth}
   \begin{center}
   \includegraphics[width=0.90\textwidth]{roadmap.png}
   \visible<2>{
   \includegraphics[width=0.50\textwidth]{PILE-UP.png}
   \begin{center}
   Empilement de 78 collisions.
   \end{center}
}
   \end{center}
   	\end{columns}
   \visible<2>
  	{
  		\begin{block}{}
  		{\color{red}Augmentation de la luminosité \\
  		$\Rightarrow$ Augmentation de l'empilement ($\sim$ 140-200 collisions par croisement de faisceaux) et du flux de particules dans les détecteurs\\
  		 $\Rightarrow$ Mise à niveau de CMS.}
  		\end{block}
  	}
	\end{frame}
    ...
    \visible<2-4>{
       ...
       \end{block}}
    	...
    	\end{tikzpicture}
    \end{columns}
	\visible<4>{\begin{block}{\centering Instrumentation des zone RE3/1 et RE4/1 (1.8<$\eta$<2.4)}
			\begin{itemize}
				\item Améliorer la résolution temporelle des traces muons.
				\item récupérer les zones moins efficaces des CSC.
				\item Augmenter la redondance dans ces zones.\\
	...
	\begin{block}{\centering Le mode avalanche}
		...
		\only<2>{\centering Ionisations primaires.}
    ...
    \begin{center}
   	\includegraphics[width=0.5\linewidth]{CMSRPC.png}\includegraphics[width=0.35\linewidth]{gaps.png}
   	\end{center}
    	\begin{itemize}
    		\item Chambre \textit{double gaps} de \SI{2}{\milli\meter}
    		\item Électrodes en Bakélite ($\rho=1-6\times10^{10}$\si{\ohm\centi\meter})
    		\item Rugosité $\sim$\SI{100}{\nano\meter}$\Rightarrow$ Traitement de surface à l'huile de lin
	...
	\begin{block}{\centering Mélange gazeux}
			\begin{itemize}
				...
				\item 40-50\% d'humidité.
			\end{itemize}
	\end{block}
	\end{frame}
	\subsection{Les improved Resistive Plate Chambers}
	\begin{frame}
	Les RPC actuelles ont été testées jusqu'à $\sim$\SI{300}{\hertz\per\square\centi\meter}.\\
	Incapables de supporter le flux de particules présent dans les zones RE3/1 et RE4/1.
	...
	\begin{block}{\centering Possibilités}
		\begin{itemize}
		\item Réduire l'épaisseur $d$ des électrodes $\rightarrow$ Nouvelles chambres en Bakélite.
		\item Réduire la charge $q$ créée par l'avalanche $\rightarrow$ Réduire l'épaisseur de gaz.
		\item Réduire la résistivité des électrodes $\rightarrow$ \textit{Low Resistivity Glass}.
	\end{itemize}
	\end{block}
	\end{frame}
	\subsection{Technologie proposée par l'IPNL}
	\begin{frame}
		\begin{block}{\centering Technologie proposée par l'IPNL}
			\begin{center}
			Combine toute les possibilités d'amélioration :
			\end{center}
			\begin{itemize}
				\item Réduire la charge $q$ créée par l'avalanche :\\
				$\rightarrow$ Réduire l'épaisseur de gas \SI{1.2}{\milli\meter} (\SI{2}{\milli\meter} CMS).\\
				$\rightarrow$ Électronique à bas bruit développée par le groupe OMEGA.
				\item Réduire l'épaisseur $d$ des électrodes :\\
				$\rightarrow$ \SI{1}{\milli\meter} (\SI{2}{\milli\meter} CMS).
				\item Réduire la résistivité des électrodes :\\
				$\rightarrow$ \textit{Low Resistivity Glass} 1 à \SI{5e10}{\ohm\centi\meter}.
			\end{itemize}
		\end{block}
	\end{frame}
	...
	\begin{columns}
		...
		\begin{block}{\centering Caractéristiques}
			\begin{itemize}
				...
				\item {\color{Green}Comportement ohmique} : {\color{Green} stable (\SI{1}{\coulomb\per\square\centi\meter})}
	...
	\begin{block}{\centering Chambre single gap}
		\begin{itemize}
			...
			\item Épaisseur du gaz \SI{1.2}{\milli\meter} (93\% TFE \chemform{C_2H_2F_4}, \num{5}\% \chemform{CO_2}, \num{2}\% \chemform{SF_6})
			\begin{center}
...
\end{frame}
    ...
    \begin{block}{\centering Éléments importants pour la caractérisation d'une chambre}
    	\begin{itemize}
    		\item \textbf{L'efficacité de détection} :  le rapport entre le nombre de particules ayant réellement été détectées et le nombre total de particules ayant traversé la chambre. 
    		\item \textbf{La multiplicité (\textit{Cluster Size}) }: Le nombre de pads ou strips touchés lorsqu’une particule traverse la chambre.
    		\item \textbf{Le bruit électronique} : La fréquence de détection par l’électronique, d’un signal considéré comme non physique. 
    		\item \textbf{Sensibilité au bruit de fond} : photons, neutrons, \ldots
    		\item \textbf{Le vieillissement} : Évolution des caractéristiques de la chambre en fonction du temps.
    	\end{itemize}
    \end{block}
	\end{frame}
	...
	\begin{block}{\centering Méthode par reconstruction de traces}
		\begin{itemize}
			...
			\item Prise en compte de la géométrie du détecteur.
			\item Présélection des candidats muon (agrégation en temps) :
				\begin{itemize}
					\item $\geq$3 Hits dans un coup d'horloge.
					\item Sélection d'une fenêtre $t_{fen}=\pm3$.
				\end{itemize}
			\item Des hits dans au moins 3 chambres
		...
		\hfill
		\includegraphics[width=0.23\textwidth]{2.png}
		\hfill
		\includegraphics[width=0.265\textwidth]{3.png}
	\end{block}
	\end{frame}
	\subsection{Programme d'analyse des données}
	\begin{frame}
			\begin{block}{\centering Test du programme d'analyse}
				Simulation GEANT4 du SDHCAL (48 GRPC) développée à l'IPNL :
				\begin{itemize}
					\item Simulation du passage de muons de \SI{100}{\giga\eV}
					\item Permet d'appliquer des cartes d'efficacités
				\end{itemize}
			\end{block}
			\begin{center}
			\includegraphics[width=0.70\textwidth]{effisimul.jpg}
			\end{center}
	\end{frame}
    ...
    \begin{frame}
    	\begin{columns}
    		...
    		\begin{block}{\centering Efficacité en fonction de la HV}
    			...
    			\begin{center}
    			\scalebox{0.43}{\includetex{HVPS}}\\
    			
    			Flux de muons : \SI{3.5}{\kilo\hertz\per\square\centi\meter}
    			\end{center}
    		\end{block}
    		\begin{block}{\centering Efficacité en fonction du flux}
    			%HV=\SI{7.2}{\kilo\volt}
    			\begin{center}
    		\scalebox{0.43}{\includetex{RatePS}}
    			\end{center}
    	\end{block}
    	\end{columns}
    \end{frame}
	\begin{frame}
		\begin{block}{\centering Multiplicité et Résolution spatiale}
			\begin{itemize}
				\item $\sigma_{1 side}=\frac{Nd}{\sqrt{12}}$
				...
				\item  $\sigma_{2 side}=\frac{D}{\sqrt{12}}$
	...
	\begin{frame}
		\begin{block}{\centering Chambres}
			...
			\end{itemize}
				...
				\begin{figure}
					\centering
					\subfloat{\includegraphics[width=.4\linewidth]{FaisceauSPS1.png}}
					\hfill
					\subfloat{\includegraphics[width=.4\linewidth]{FaisceauSPS2.png}}
				\end{figure}
			\end{columns}
		\end{block}
	...
	\begin{block}{\centering Taux de conversion $\gamma$/$e^{-}$}
		...
		\end{center}
			\begin{center}
			$\Rightarrow$ 46500 $e^{-}$\si{\per\square\centi\meter\per\second}
		\end{center}
	\end{block}
	\end{columns}
	\end{frame}
	...
	La saturation arrive vers \SI{700}{\hertz\per\square\centi\meter} (1/ATT=0.015) pour la float glass.\\
	La chambre en verre de basse résistivité continue de fonctionner $\Rightarrow$ Possibilité de récupérer la perte d'efficacité en augmentant la HV. }
	\end{center}
	\end{frame}
	...
	\begin{block}{\centering Charges intégrées en fonction du nombre de jours effectifs}
			...
			\vspace*{-0.6cm}
			\begin{tikzpicture} 
			\node [inner sep=0pt,above right]{\includegraphics[width=0.98\textwidth]{DIF32_HR_5.png}};
		...
		\begin{center}
			...
			\vspace*{-0.3cm}
			\begin{tikzpicture} 
			\node [inner sep=0pt,above right]{\includegraphics[width=0.98\textwidth]{DIF16_LR_5.png}};
...
\end{block}	
	...
	$\Rightarrow$ Construire des chambres de type RE1/1 (\SI{1.1}{\meter}$\times$\SI{60}{\centi\meter})
	\end{center}
		\begin{block}{\centering Méthode par collage}
				\begin{columns}
				\column{.45\textwidth}
				\begin{center}
					\includegraphics[width=0.42\textwidth]{Glass-glued.png}
				\end{center}
				\column{.45\textwidth}
				\begin{center}
				\includegraphics[width=0.60\textwidth]{Glass-glued2.png}
				\end{center}
				\end{columns}
				\begin{center}
...
\begin{columns}
	...
	\begin{block}{\centering Améliorations}
		\begin{itemize}
			...
			\item Meilleure masse entre les FEB et le capot
...
\begin{block}{\centering Efficacité en fonction de la HV}
	...
	\end{center}
	\begin{center}
		5\% d'inefficacité en tenant compte des effets d'acceptance géométrique.
	\end{center}
\end{block}
\end{frame}
\begin{frame}
...
\begin{block}{\centering Probabilité de streamer}
	\begin{center}
		Probabilité définit comme la probabilité d'avoir un cluster de taille supérieure à 7.
	\end{center}
\begin{center}
...
\end{block}
\end{frame}
\subsection{Étude du bruit en fonction de la proportion de \chemform{SF_6}}
\begin{frame}
%\begin{columns}
%	\column{0.5\textwidth}
	\begin{block}{\centering Courant dans la chambre RE1/1 vs \% \chemform{SF_6}}
		\begin{center}
		\scalebox{0.51}{\includetex{CurrentRE11}}
		\end{center}
	\end{block}
%	\column{0.5\textwidth}
	\begin{block}{\centering Taux de bruit dans la chambre RE1/1 vs \% \chemform{SF_6}}
		\begin{center}
		\includegraphics[width=0.70\textwidth]{NoisvsSF6.png}
		\end{center}
		
	\end{block}
%\end{columns}
\end{frame}
\subsection[Résumé des expériences pour les électrodes en verre de basse résistivité]{Résumé des tests pour les LR-GRPC}
\begin{frame}
\begin{block}{\centering Résumé des expériences pour les électrodes en verre de basse résistivité}
\begin{itemize}
	\item Les verres de basse résistivité sont capables de supporter le flux présent dans les zones RE3/1 et RE4/1.
	\item Il est possible de construire des chambres de grandes dimensions avec les verres de basse résistivité de 32$\times$30\si{\square\centi\meter} (2 méthodes).
	\item Le pourcentage de SF6 semble déterminant pour ce type de chambre.
	\item Le vieillissement des chambres doit être poursuivi.
\end{itemize}
\end{block}
\end{frame}
	...
	\begin{itemize}
		...
		$T$ le temps d'arrivée du signal sur le strip\\ $v$ la vitesse de propagation du signal.
	\end{itemize}
	\begin{center}
	\includegraphics[width=0.40\textwidth]{PCB1.png}
	\end{center}
	\end{block}
	\end{frame}
	...
	\end{block}
		\begin{columns}
			...
			\begin{block}{\centering Électronique}
				\begin{itemize}
					\item 2 PETIROC (32 voies chacun).
					\begin{center}
...
\end{frame}
	...
	\begin{block}{\centering Conclusion}
		\begin{itemize}
			\item Les verres de basse résistivité sont capables de supporter le flux présent dans les zones RE3/1 et RE4/1.
			\item Il est possible de construire des chambres de grandes dimensions avec les verres de basse résistivité de 32$\times$\SI{30}{\square\centi\meter} (2 méthodes).
			\item Le pourcentage de SF6 semble déterminant pour ce type de chambre.
			\item La nouvelle électronique permet d'obtenir la position le long des strips.
			\item Résolution spatiale le long des strip de (\SI{1.8}{\centi\meter}).
		\end{itemize}
	\end{block}
\end{frame}
\begin{frame}
	\begin{block}{Perspectives}
		\begin{itemize}
			\item Tester les verres de basse résistivité avec des éco-gaz (\chemform{CF_3I}, HFO-1234yf)
			\item Abaisser la gamme dynamique du PETIROC (\SI{10}{\femto\coulomb})
			\item Remplacer les TDC actuels par de ASICs (voire les intégrer au PETIROC).
			\item Des tests de prototypes taille réelle sont déjà en cours.
			\begin{center}
			\includegraphics[width=0.45\textwidth]{proto.png}
			\end{center}
		\end{itemize}
	\end{block}
	\end{frame}
