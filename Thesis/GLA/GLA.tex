...
Les courbes de la figure \ref{seuils} n'étant pas linéaires sur toute la plage, nous avons veillé à ne pas dépasser la zone non linéaire lors de nos tests en faisceaux.
...
\begin{minipage}[th!]{1\textwidth}
	...
	\begin{minipage}[th!]{1\textwidth}
	    ...
	    \captionsetup{type=subfigure}\caption{À \SI{2}{\meter} de la source. Le facteur d'atténuation de la source vaut \num{220}.}
	    \label{bla1}
	    \begin{minipage}[th!]{0.49\textwidth}
	    	\noindent
	    	\centering
	...
	\begin{minipage}[th!]{1\textwidth}
	    ...
	    \renewcommand\thesubfigure{(\alph{subfigure})}
	    \vspace*{-0.2cm}
	    \captionsetup{type=subfigure}\caption{À \SI{5.5}{\meter} de la source. Le facteur d'atténuation de la source vaut \num{150}.}
	    \label{bla3}
		\hfill
	    \begin{minipage}[th!]{0.49\textwidth}
	    	\noindent
...
\begin{figure}[!ht]
	\centering
	\subfloat[Valeur de l'atténuateur en fonction du nombre de jours depuis le 1\ier août 2016.\label{a}]{\includegraphics[width=0.82\linewidth]{GLA/current/DIF16_LR_4.png}}
\hfill
\subfloat[Tension appliquée en fonction du nombre de jours depuis le 1\ier août 2016.\label{b}]{\includegraphics[width=0.82\linewidth]{GLA/current/DIF16_LR_3.png}}
\\
\subfloat[Courant dans la chambre en fonction du nombre de jours depuis le 1\ier août 2016.\label{c}]{\includegraphics[width=0.82\linewidth]{GLA/current/DIF16_LR_1.png}}
\hfill
\subfloat[Charge intégrée en fonction du nombre de jours depuis le 1\ier août 2016.\label{d} ]{\includegraphics[width=0.82\linewidth]{GLA/current/DIF16_LR_2.png}}
\\
\subfloat[Charge intégrée en fonction du nombres de jours effectifs.\label{e}]{\includegraphics[width=0.82\linewidth]{GLA/current/DIF16_LR_5.png}}
...
La chute rapide du courant en fonction du temps montre un vieillissement prématuré des chambres. Le test en faisceaux de mai 2016 a permis de comprendre les causes de ce vieillissement prématuré.
...
La résistivité des chambres de basse résistivité est beaucoup plus élevée qu'attendue ($\sim$\SI{e10}{\ohm.\centi\meter}). Toutes ces constatations (cf.Fig~\ref{CourantLR},cf.Fig~\ref{struc},cf.Table~\ref{resistivite}) nous ont amené à ouvrir une chambre afin d'essayer de comprendre ces résultats.
\vspace{0.4cm}
\newpage
...
La première méthode consiste à coller les verres entre eux grâce à de la colle RHODORSIL\textsuperscript{\textregistered}
 ...
 \begin{figure}[ht!]
 	...
 	\centering
 	\vspace*{1.cm}
 	\subfloat[\textit{Gap(i)} hermétique formé par collage des verres. On distingue clairement les zones de collage.]{\includegraphics[width=1\textwidth]{GLA/Glass-glued.png}\label{a}}
 	\end{minipage}
    \hfill
    \begin{minipage}[t]{.45\textwidth}
    \centering
    \subfloat[\textit{Gap(ii)} et \textit{gap(iii)} hermétiques formés par collage des verres dans la cassette, avec les \textit{strips} et le \textit{gap(i)} au dessous.]{\includegraphics[width=1\textwidth]{GLA/Glass-glued2.png}\label{b}}
    \\
    \vspace*{1cm}
...
\newpage
Le taux de bruit moyen pour la partition $B$ de la chambre RE1/1 en fonction de la tension pour différentes proportions de \chemform{SF_6} est montré \ref{bruitB}.
 ...
 Le taux de bruit de la partition $B$ en fonction de la tension pour plusieurs seuils dans le cas du mélange de gaz à 5\% et 3\% de \chemform{SF_6} est présenté figure \ref{bruitB2}.
  
\begin{figure}[ht!]
	\centering
	\subfloat[\label{ta}]{\includegraphics[width=0.705\textwidth]{GLA/Noise5p100.png}} 
	\hfill
	\subfloat[\label{tb}]{\includegraphics[width=0.705\textwidth]{GLA/Noise3p100.png}}
...
\begin{figure}[ht!]
	\centering
	\subfloat[Efficacité et taux de bruit pour la chambre en verre de basse résistivité.\label{tta}]{\scalebox{0.95}{\includetex{GLA/EffiNoiseChi}}\label{effi904Chi}} 
	\\
	\vspace*{-0.2cm}
	\subfloat[Efficacité et taux de bruit pour la chambre en Bakélite.\label{ttb}]{\scalebox{0.95}{\includetex{GLA/EffiNoiseBak}}\label{effi904Bak}}
	\caption{Efficacité (vert) et taux de bruit (bleu) pour la chambre en verre de basse résistivité (\textcolor{gray}{a}) et en Bakélite (\textcolor{gray}{b}) pour le mélange de gaz CMS. Le taux de bruit de la chambre de basse résistivité pour le mélange à 5\% de \chemform{SF_6} est également donné (rouge). Le seuil appliqué est de \SI{72}{\femto\coulomb}.}
\end{figure}  
