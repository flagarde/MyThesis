...
La figure \ref{lhcschema} est une vue schématique du LHC. En vérité le LHC n'est pas parfaitement circulaire, mais est composé d'octants (cf.Fig~\ref{octants}) qui comporte chacun une section droite de longueur \SI{528}{\meter} et un secteur courbe d'une longueur de \SI{2.45}{\kilo\meter}. 
...
\captionof{figure}{Vue transversale d'un dipole du LHC.}
\label{dipole}	
\end{minipagewithmarginpars}
...
\end{equation}
où $\epsilon_{n}=\epsilon\gamma\beta$ est l'émittance normalisée, $\gamma$ le facteur de \bsc{Lorentz} $\beta=v/c$.
...
\begin{minipagewithmarginpars}[ht!]{0.95\textwidth}
	...
	\captionof{figure}{$x$ fois la  distribution de parton non-polarisé $f(x)$ ou $f=u_{v}$, $d_{v}$, $\bar{u}$, $\bar{d}$, $s$, $c$, $b$, $g$ et leurs incertitudes en utilisant la paramètrisation NNLO MSTW2008 à l'échelle \SI{10}{\square\giga\eV} (gauche) \SI{10000}{\square\giga\eV} (droite).}
	\label{pdf}	
\end{minipagewithmarginpars}
...
\begin{itemize}[label=$\bullet$]
 ...
 \item De nouvelles lignes de transmission électrique vont également être installées. Ces câbles seront faits d'un matériau supraconducteur \chemform{MgB_2} (diborure de magnésium) capable, de fonctionner à haute température (\SI{20}{\kelvin}), beaucoup plus stables que les supraconducteurs conventionnels. Ils permettront de porter des courants de plus de \SI{100}{\kilo\ampere}.
...
\captionof{figure}{Schéma montrant les différents types de mise à niveau nécessaires au HL-LHC.}
\label{man}	
\end{figure}
