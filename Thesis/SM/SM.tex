...
%\vspace*{-0.25cm}
%\begin{center}\normalfont\footnotesize {Le Tétraèdre (le Feu).}\end{center}
%\vspace*{-0.25cm}
\includegraphics[width=0.25\marginparwidth]{SM/Hexahedron.png}
\captionsetup{type=subfigure}\caption{Le Cube (la Terre).}
%\vspace*{-0.25cm}
%\begin{center}\normalfont\footnotesize {Le Cube (la Terre).}\end{center}
%\vspace*{-0.25cm}
\includegraphics[width=0.25\marginparwidth]{SM/Octahedron.png}
\captionsetup{type=subfigure}\caption{L'Octaèdre (l'Air).}
%\vspace*{-0.25cm}
%\begin{center}\normalfont\footnotesize {L'Octaèdre (l'Air).}\end{center}
%\vspace*{-0.25cm}
\includegraphics[width=0.25\marginparwidth]{SM/Icosahedron.png}
\captionsetup{type=subfigure}\caption{L'Icosaèdre (l'Eau).}
%\vspace*{-0.25cm}
...
\bsc{Anaxagore} ($\sim$\num{500}--\num{428} av.J.-C.) prônait que toute chose est formée de particules élémentaires. Cette idée sera reprise par \bsc{Empédocle} ($\sim$\num{495}--$\sim$\num{435} av.J.-C.) qui proposa l'eau, la terre, l'air et le feu comme étant ces particules. \bsc{Platon} (\num{428}/\num{427}--\num{348}/\num{347} av.J.-C.) associera ces quatre éléments aux polygones réguliers convexes de l'espace à trois dimensions (le tétraèdre pour le Feu, le cube pour la Terre, l'octaèdre pour l'Air, l'icosaèdre pour l'Eau, le dodécaèdre quant à lui représente l'Éther, élément constituant l'Univers (cf.Fig~\ref{solides})). On doit à \bsc{Leucippe} ($\sim$\num{460}--$\sim$\num{370} av.J.-C.) et son disciple \bsc{Démocrite} \hbox{($\sim$\num{460}--$\sim$\num{370} av.J.-C.)} le concept d'atomes, indivisibles et séparés par du vide, qui composent la matière. La véracité de l'atomisme fera débat pendant plus de deux mille ans et ne sera validée expérimentalement qu'au cours du XIX\ieme siècle.

 Parmi les travaux les plus importants qui prouveront l'existence des atomes, citons ceux de \bsc{Lavoisier} (\num{1743}--\num{1794}) qui décompose de nombreuses substances en "Éléments". De multiples travaux sur les gaz, la cristallographie, la physique statistique et la thermodynamique, dus à \bsc{Bernoulli} (\num{1700}--\num{1782}) pour la cinétique des gaz, \bsc{Haüy} (\num{1743}--\num{1822}) pour qui la forme des cristaux reflète la symétrie des "briques élémentaires" les constituant, \bsc{Dalton} (\num{1766}--\num{1844}) avec sa symbolisation des corps simples et des corps composés par des symboles auxquels il donne un poids de matière (cf.Fig~\ref{atom}), et la liste des masses atomiques d'un certain nombre d'éléments rapportées à la masse de l'hydrogène, \hbox{\bsc{Gay-Lussac} (\num{1778}--\num{1850})} qui montre que les rapports des volumes des réactifs et des produits de réaction sont des nombres entiers petits, \bsc{Maxwell} (\num{1831}--\num{1879}) et la dispersion statistique des vitesses des molécules, \bsc{Boltzmann} (\num{1844}--\num{1906}) et la répartition statistique des vitesses dans un gaz, \bsc{Mendeleïev} et sa classification périodique des éléments et sa prédiction de nouveaux atomes (cf.Fig~\ref{periodique}). Ces travaux feront passer petit à petit cette théorie en réalité scientifique. 
...
\marginpar
{
    ...
    \captionsetup{type=figure}\caption{Un baryon ($p$).}
    \label{baryons}
}
...
\marginpar
{
	...
	\captionsetup{type=figure}\caption{Schéma d'une gerbe atmosphèrique.}
	\label{gerbe}
}
...
\begin{table}[H]
	...
	\begin{tabular}{|O|O|O|O|N}
		...
		\cellcolor{Green2} Notation & \cellcolor{Green} $e^{-},e^{+}$ & \cellcolor{Green} $\mu^{-},\mu^{+}$ & \cellcolor{Green} $\tau^{-},\tau^{+}$ \\
...
\marginpar
{
	...
	\captionsetup{type=subfigure}\caption{Électromagnétisme : la foudre.}
...
\begin{table}[ht!]
	...
	\begin{tabular}{|O|O|O|O|N}
		...
		\cellcolor{Red}\shortstack{\SI{1e-15}{\meter}}& 
		...
		\cellcolor{Red}\shortstack{\SI{1e-18}{\meter}}& 
...
\begin{table}[H]
	...
	\hline
	\cellcolor{Orange}Gauches $Q_{\alpha L}^{i}$ &\cellcolor{Orange} $
	\left(\begin{matrix} 
	 u^{i}\\
	 d^{i} 
	...
	\end{matrix}\right)_{L}$ \\
	\hline
	\cellcolor{Orange}Droits $Q_{\beta R}^{i}$&\cellcolor{Orange}$ u_{iR},\ d_{iR},\ c_{iR},\ s_{iR},\ t_{iR},\ b_{iR}$\\
	\hline
	\rowcolor{Green2}\multicolumn{2}{|Cc|}{Leptons} \\
	\hline
	\cellcolor{Green}Gauche $L_{\alpha L}$& \cellcolor{Green}$
	\left(\begin{matrix} 
	\nu_{e}\\
	 e^{-}
	\end{matrix}\right)_{L}$,$\ 
	...
	\end{matrix}\right)_{L} $\\
	\hline
	\cellcolor{Green}Droits $L_{\gamma R}$& \cellcolor{Green}$e^{-}_{R},\ \mu_{R},\ \tau_{R},\ \left(\nu_{e R},\ \nu_{\mu R},\ \nu_{\tau R}\right)$ \\
	\hline
\end{tabular}
...
\end{pmatrix}
\end{equation*}
\captionsetup{type=figure}\caption{Les matrices canoniques de \bsc{Pauli}.}
\label{Pauli}
}
\marginpar
{
...
\end{pmatrix}
\end{equation*}
\captionsetup{type=figure}\caption{Les matrices canoniques de \bsc{Gell-Mann}.}
\label{Gell-Mann}
}
...
\end{equation}
et en développant le doublet $H$ autour de cet état du champ de \bsc{Higgs} qui brise la symétrie $SU(2)_{L}\times U(1)_{Y}$ on trouve : 
\begin{equation}
...
\captionsetup{type=figure}\caption{Corrections radiatives du premier ordre pour le boson de \bsc{Higgs}.}
\label{corrections}
\end{figure}
...
\captionsetup{type=figure}\caption{Le satellite \bsc{Plank}.}
\label{Plank}
} 
